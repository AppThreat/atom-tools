...
def test_code_to_routes():
    ...
    assert code_to_routes(
        """Rails.application.routes.draw do root \"home#index\" get \"/articles\", to: \"articles#index\" get '/oauth2-callback', to: 'o_auth#oauth_callback' get '/logout', to: 'o_auth#logout' get '/login', to: 'o_auth#login' get '/register', to: 'o_auth#register' get '/endsession', to: 'o_auth#endsession' end"""
    ) == [
...
def test_code_to_routes_sidekiq():
    assert code_to_routes(
        ...
        HttpRoute(url_pattern="/admin/user_development_notifications", method="GET"),
        HttpRoute(url_pattern="/admin/marketing_campaign_scheduler", method="GET"),
        HttpRoute(url_pattern="/admin/schedule_marketing_campaign", method="POST"),
...
def test_code_to_routes_scope_dangling():
    assert code_to_routes(
        """resources :consignment_requests, only: [:show] do member do get :print put 'state', to: :update_state, as: :update_state end end"""
    ) == [
        HttpRoute(url_pattern="/consignment_requests/{print}", method="GET"),
        HttpRoute(url_pattern="/consignment_requests/{id}/state", method="PUT"),
    ]
    # Needs development
    assert code_to_routes(
        """scope \"payments_gift_card_infos\", :controller => \"admin_payments_gift_card_infos\" do match \"(:gift_card_fingerprint)"""
    ) == [
        HttpRoute(
            url_pattern="/payments_gift_card_infos/{gift_card_fingerprint}",
            method="GET",
        )
    ]
    assert code_to_routes(
        """scope \"payments_upi_infos\", :controller => \"admin_payments_upi_infos\" do match \"(:upi_info_id)"""
    ) == [HttpRoute(url_pattern="/payments_upi_infos/{upi_info_id}", method="GET")]
    assert code_to_routes(
        """scope \"payments_venmo_infos\", :controller => \"admin_payments_venmo_infos\" do match \"(:venmo_info_id)"""
    ...
    assert code_to_routes(
        """scope 'influencer_campaigns', :controller => 'admin_influencer_program_campaigns' do get 'list_jobs', :action => 'list_jobs', :as => 'list_jobs' post 'download_jobs', :action => 'download_jobs', :as => 'download_jobs' end"""
    ) == [
        HttpRoute(url_pattern="/influencer_campaigns/list_jobs", method="GET"),
...
w_bulk_account_action', :via => [:get, :post] end""") == [
        HttpRoute(url_pattern="/bulk_account_actions/summary", method="GET"),
        HttpRoute(url_pattern="/bulk_account_actions/new", method="GET"),
    ]
    assert code_to_routes(
        """scope \"templates\", :controller => 'email_templates' do get 'edit', :action => 'edit', :as => 'email_template_edit' end"""
    ) == [HttpRoute(url_pattern="/templates/edit", method="GET")]
    assert code_to_routes(
        """member do get :print put 'state', to: :update_state, as: :update_state end"""
    ) == [
        HttpRoute(url_pattern="/{print}", method="GET"),
        HttpRoute(url_pattern="/{id}/state", method="PUT"),
    ]
    assert code_to_routes("""scope \"posts/(:post_id)""") == []
    assert code_to_routes(
        """resources :email_triggers do get 'show_changelog', controller: 'email_triggers', action: 'show_changelog', as: 'show_changelog' end"""
    ) == [
        HttpRoute(url_pattern="/email_triggers", method="GET"),
    ...
    assert code_to_routes(
        """scope \"reports\", :controller => \"admin_user\" do post \"approve_report\" , :action => 'submit_approve_report', :as => 'submit_approve_report' post \"ignore_report\" , :action => 'submit_ignore_report', :as => 'submit_ignore_report' end"""
    ) == [
        HttpRoute(url_pattern="/reports/approve_report", method="POST"),
        HttpRoute(url_pattern="/reports/ignore_report", method="POST"),
    ]
    assert (
        code_to_routes("""scope :controller => \"admin_experiences\" do get 'markets', :action => 'list_experiences', :as => 'list_experiences' get 'markets/:short_name', :action => 'admin_view_experience', :as => '
...
ets/:short_name/web_nav', :action => 'market_web_nav_config', :as => 'update_market_web_nav_config' get 'markets/web_nav/get_showrooms', :action => 'get_showrooms_for_nav', :as => 'get_showrooms_for_nav' end""")
        == [
            HttpRoute(url_pattern="/markets", method="GET"),
            HttpRoute(url_pattern="/markets/{short_name}", method="GET"),
            HttpRoute(url_pattern="/markets/{short_name}", method="POST"),
            HttpRoute(url_pattern="/markets/{short_name}/web_nav", method="GET"),
            HttpRoute(url_pattern="/mark", method="POST"),
            HttpRoute(url_pattern="/markets/web_nav/get_showrooms", method="GET"),
        ]
    )
    assert (
        code_to_routes("""scope :controller => \"admin_feature_settings\" do get \"list_feature_settings\", :action => 'list_feature_settings', :as => 'list_feature_settings' get \"editable_feature_settings\", :acti
...
/:feature_id/variations_list\", :action => 'list_variations', :as => 'variations_list', :via => [:get, :post] get 'feature_settings/:feature_id/show_mappings_hi...""")
        == [
            HttpRoute(url_pattern="/list_feature_settings", method="GET"),
            ...
            HttpRoute(
                url_pattern="/feature_settings/{feature_id}/show_mappings_hi",
                method="GET",
            ),
        ]
    )
    assert code_to_routes(
        """scope :controller => 'admin_service_flags' do get 'list_service_flags', :action => 'list_service_flags', :as => 'list_service_flags' end"""
    ) == [HttpRoute(url_pattern="/list_service_flags", method="GET")]


def test_code_to_routes_scope_multiple_path():
    assert code_to_routes(
        """scope \"inventory_unit/:inventory_unit_id\" do post \"clear_inventory_unit\" post \"received_inventory_unit\" post \"send_shipping_reminder\" post \"send_final_shipping_reminder\" post \"return_shipping_reminder\" post \"send_cancel_order_notice\" post \"send_buyer_shipping_delayed\" post \"send_buyer_shipping_really_delayed\" post \"send_concierge_delayed\" post \"shipped_inventory_unit\" post \"item_shipped_email\" post \"buyer_accept_order_reminder_email\" post \"delivered_inventory_unit\" post \"concierge_delivered_inventory_unit\" post \"concierge_shipped_inventory_unit\" post \"return_shipped_inventory_unit\" post \"item_delivered_email\" post \"send_notice_left\" post \"order_not_delivered\" end"""
    ) == [
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/clear_inventory_unit",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/received_inventory_unit",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/send_shipping_reminder",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/send_final_shipping_reminder",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/return_shipping_reminder",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/send_cancel_order_notice",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/send_buyer_shipping_delayed",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/send_buyer_shipping_really_delayed",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/send_concierge_delayed",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/shipped_inventory_unit",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/item_shipped_email",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/buyer_accept_order_reminder_email",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/delivered_inventory_unit",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/concierge_delivered_inventory_unit",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/concierge_shipped_inventory_unit",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/return_shipped_inventory_unit",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/item_delivered_email",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/send_notice_left",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/inventory_unit/{inventory_unit_id}/order_not_delivered",
            method="POST",
        ),
    ]
    assert code_to_routes(
        """scope \"payments_card_infos\", :controller=> \"admin_payments_card_infos\" do match \"(:card_info_id)"""
...
def test_code_to_routes_collection():
    assert code_to_routes(
        'collection do post "update_dd_info" post "decrypted_bank_acct_num" end'
    ) == [
...
def test_code_to_routes_advanced():
    assert code_to_routes(
        "scope :controller=>'search_v2' do get 'brand/:brand', :action => 'listings_by_brand', :as => 'search_by_brand', :constraints => {:brand => /[^//]+/}"
    ) == [HttpRoute(url_pattern="/brand/{brand}", method="GET")]
    assert code_to_routes("def resource (...)") == []
    assert code_to_routes('resources :photos do member do get "preview" end end') == [
        HttpRoute(url_pattern="/photos", method="GET"),
...
def test_code_to_routes_large():
    assert code_to_routes(
        ...
        HttpRoute(url_pattern="/list_swap_orders", method="GET"),
    ...
    assert code_to_routes("""Web::Application.routes.draw do
        ...
        HttpRoute(
            url_pattern="/admin/swap_order/{swap_order_id}/view_swap_order",
            method="GET",
        ),
        HttpRoute(
            url_pattern="/admin/swap_order/{swap_order_id}/buyer_swap_order_reminder",
            method="POST",
        ),
        HttpRoute(
            url_pattern="/admin/swap_order/{swap_order_id}/list_swap_orders",
            method="GET",
        ),
...
def test_code_to_routes_sinatra():
    ...
    assert code_to_routes(
        "get '/:model' do mod = @models[params[:model].to_sym] @model = mod.to_s.split( '::' )"
    ) == [HttpRoute(url_pattern="/{model}", method="GET")]
    assert code_to_routes(
        "get '/:model/:id' do mod = @models[params[:model].to_sym] @details = mod.find( params[:id] )"
    ) == [HttpRoute(url_pattern="/{model}/{id}", method="GET")]
    assert code_to_routes(
        "get '/:model/:id/delete' do @item = @models[params[:model].to_sym].find( params[:id] )"
    ) == [HttpRoute(url_pattern="/{model}/{id}/delete", method="GET")]
    assert code_to_routes(
        "options '*' do response.headers['Access-Control-Allow-Origin'] = '*' response.headers['Access-Control-Allow-Methods'] = 'HEAD,GET,PUT,DELETE,OPTIONS' response.headers['Access-Control-Allow-Headers'] = 'X-Requested-With, X-HTTP-Method-Override, Content-Type, Cache-Control, Accept' halt 200 end"
    ) == [HttpRoute(url_pattern="/", method="OPTIONS")]
    assert code_to_routes(
        "post '/:model/:id' do mod = @models[params[:model].to_sym] @details = mod.find( params[:id] )"
    ) == [HttpRoute(url_pattern="/{model}/{id}", method="POST")]
    assert code_to_routes(
        "post '/:model' do mod = @models[params[:model].to_sym] @model = mod.to_s.split( '::' )"
...
def test_endpoints_to_routes():
    assert endpoints_to_routes("api.restsite.com", "initialize") == [
        HttpRoute(url_pattern="/", method="GET", servers=["api.restsite.com"])
    ]
    assert endpoints_to_routes("/users", "initialize") == [
        HttpRoute(url_pattern="/users", method="GET", servers=None)
    ]
    assert endpoints_to_routes("/users/1", "initialize") == [
        HttpRoute(url_pattern="/users/1", method="GET", servers=None)
    ]
    assert endpoints_to_routes('"http://google.com/"', "parse") == [
    ...
    assert endpoints_to_routes("http://mysite.com/some_api", "parse") == [
        HttpRoute(url_pattern="/some_api", method="GET", servers=["http://mysite.com"])
    ]
