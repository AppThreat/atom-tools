...
def test_determine_operations_no_responses_arg_returns_empty_responses():
    ...
    call = {
        "callName": "get",
        "resolvedMethod": '@GetMapping("/test")',
        "paramTypes": [],
    }
...
def test_determine_operations_none_responses_treated_as_empty():
    call = {
        "callName": "post",
        "resolvedMethod": '@PostMapping("/test")',
        "paramTypes": [],
    }
...
def test_infer_non_java_returns_empty(js_converter):
    result = js_converter._infer_java_response_codes("some/file.js", 10, "get")
    assert result == {}
...
def test_infer_defaults_to_http_method_when_no_response_entity(
    java_response_codes_converter,
):
...
def test_full_converter_post_with_response_entity_created(
    java_response_codes_converter,
):
    ...
    for path, path_item in result.get("paths", {}).items():
        for method in ("get", "post", "put", "patch", "delete", "head", "options"):
            if method in path_item:
                ...
                assert responses != {}, f"Empty responses for {method.upper()} {path}"
