...
def code_to_routes(code: str) -> List[HttpRoute]:
    ...
    if (
        not code
        or code.startswith("Given ")
        or code.startswith("When ")
        or code.startswith("Then ")
    ):
        return []
    ...
    if (
        "namespace" not in code
        and "collection" not in code
        and code.count("resources") <= 1
        and code.count("get") <= 1
        and "only" not in code
    ):
    ...
    for i, part in enumerate(code_parts):
        ...
        if is_in_resource_block:
            ...
            if (
                part in ("resources",)
                and namespace_url_prefix
                and namespace_url_prefix != resources_url_prefix
            ):
                resources_url_prefix = namespace_url_prefix + "/".join(
                    resources_url_prefix.split("/")[:-1]
                )
        ...
        if (
            ...
            and part in ("resource", "resources", "namespace", "member")
            ...
            if len(code_parts) > i + 3 and code_parts[i + 2] in (
                "path:",
                "path",
                "path(",
            ):
                url_pattern = _clean_url(code_parts[i + 3].replace('"', ""))
                routes += _get_dangling_routes(
                    i, part, code, code_parts, f"{scope_url_prefix}/{url_pattern}/"
                )
                continue
            if len(code_parts) > i + 2 and code_parts[i + 2] in (
                "resources",
                "resource",
                "get",
                "post",
                ...
                if (
                    ...
                    url_prefix_to_use = (
                        ...
                        if namespace_url_prefix
                        and url_pattern
                        and namespace_url_prefix != url_pattern
                        else namespace_url_prefix
                    )
            continue
        if (
            part in ("collection", "member", "concern", "do", "as:", "constraints:")
            or part.startswith(":")
            or part.startswith('"')
        ):
            continue
        ...
        if part in ("match",):
            fmt_path = fix_url_params(
                code_parts[i + 1].replace('"', "").replace("'", "")
            ).removesuffix(",")
            full_path = (
                fmt_path
                if not url_prefix_to_use and fmt_path in ("/", "*")
                else f"""{url_prefix_to_use}/{fmt_path.removeprefix("/")}"""
            )
        ...
        if part in HTTP_METHODS:
            for m in HTTP_METHODS:
                if (
                    ...
                    and len(code_parts) > i + 1
                    and (
                        code_parts[i + 1].startswith(":")
                        or code_parts[i + 1].startswith('"')
                        or code_parts[i + 1].startswith("'")
                    )
                ):
                    fmt_path = fix_url_params(
                    ...
                    routes.append(
                        HttpRoute(
                            url_pattern=_clean_url(full_path),
            ...
            for i, part in enumerate(code_parts):
                for m in ("resource", "resources"):
                    if (
                        ...
                        and (
                            i == len(code_parts) - 2
                            or (len(code_parts) > i + 2 and code_parts[i + 1] != "do")
                        )
                    ):
                        routes += _get_dangling_routes(
                            i,
...
def endpoints_to_routes(endpoint: str, called_method: str) -> List[HttpRoute]:
    ...
    routes.append(
        HttpRoute(
            url_pattern=_clean_url(full_path),
            method=http_verb,
            servers=[base_url] if base_url else None,
        )
    )
    return routes
